...
// const std::string MODEL_PATH = "models/Helmet.obj";
// const std::string TEXTURE_PATH = "textures/Helmet_1.png";

const std::string MODEL_PATH = "models/Garuda_and_Vishnu.obj";
const std::string TEXTURE_PATH = "textures/Garuda_and_Vishnu_1.png";
...
const bool DYNAMIC_STATES_FOR_VIEWPORT_SCISSORS = true;
...
private:
  ...
  static void loadModel(std::vector<Vertex> &vertices,
    ...
    Vertex max_vertex = {.pos = {0.0f, 0.0f, 0.0f}};
    ...
    for (const auto &shape : shapes) {
      for (const auto &index : shape.mesh.indices) {
        ...
        if (uniqueVertices.count(vertex) == 0) {
          ...
          float max_dim1 =
              std::max(std::abs(vertex.pos.x), std::abs(vertex.pos.y));
          max_dim1 = std::max(max_dim1, std::abs(vertex.pos.z));
          float max_dim2 =
              std::max(std::abs(max_vertex.pos.x), std::abs(max_vertex.pos.y));
          max_dim2 = std::max(max_dim2, std::abs(max_vertex.pos.z));

          if (max_dim2 < max_dim1) {
            max_vertex = vertex;
          }
    ...
    std::cout << "MAX_VERTEX: " << max_vertex.pos.x << "," << max_vertex.pos.y
              << "," << max_vertex.pos.z << std::endl;
  }
  ...
  static void updateUniformBuffer(VkPhysicalDevice physicalDevice,
    ...
    ubo.view =
        glm::lookAt(glm::vec3(500.0f, 500.0f, 500.0f),
                    glm ::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));

    ubo.proj = glm::perspective(
        glm::radians(25.0f),
        swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 5000.0f);
