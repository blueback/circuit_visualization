...
private:
  ...
  */
  ...
  void drawFrame2(VkPhysicalDevice physicalDevice_unp,
                  ...
                  VkCommandBuffer renderCommandBuffer_unp,
                  VkCommandBuffer copyCommandBuffer_unp,
                  VkSemaphore renderingFinishedSemaphore_unp,
                  VkFence inFlightFence_unp, VkFence interDeviceFence_unp,
                  VkBuffer stagingBuffer_unp, VkPhysicalDevice physicalDevice_p,
                  VkDevice logicalDevice_p, VkFramebuffer frameBuffer_p,
                  VkCommandBuffer copyCommandBuffer_p,
                  const bool isDynamicViewPortAndScissor) {

    vkWaitForFences(logicalDevice_unp, 1, &inFlightFence_unp, VK_TRUE,
                    UINT64_MAX);
    vkResetFences(logicalDevice_unp, 1, &inFlightFence_unp);
    ...
    submitInfo_unp.pCommandBuffers = &renderCommandBuffer_unp;
    VkSemaphore signalSemaphores_unp[] = {renderingFinishedSemaphore_unp};
    submitInfo_unp.signalSemaphoreCount = 1;
    submitInfo_unp.pSignalSemaphores = signalSemaphores_unp;
    ...
    submitInfo_unp.pWaitDstStageMask = nullptr; // Optional
    submitInfo_unp.commandBufferCount = 1;
    submitInfo_unp.pCommandBuffers = &copyCommandBuffer_unp;
    ...
    //////////////////////////////////////////////////////////////////////
    ///                   TODO : START copy to presenter
    //////////////////////////////////////////////////////////////////////

    vkResetCommandBuffer(copyCommandBuffer_p, 0);

    recordCommandBufferForCopyingFromBuffer(physicalDevice_p, logicalDevice_p,
                                            copyCommandBuffer_p, extent,
                                            stagingBuffer_p);
    ...
    submitInfo_unp.pWaitDstStageMask = nullptr; // Optional
    submitInfo_unp.commandBufferCount = 1;
    submitInfo_unp.pCommandBuffers = &commandBuffer;
    VkSemaphore signalSemaphores[] = {renderingFinishedSemaphore};
    submitInfo_unp.signalSemaphoreCount = 1;
    submitInfo_unp.pSignalSemaphores = signalSemaphores;

    if (vkQueueSubmit(graphicsQueue, 1, &submitInfo_unp, interDeviceFence) !=
        VK_SUCCESS) {
      throw std::runtime_error("failed to submit draw command buffer!");
    } else {
      std::cout << "Submitted draw command buffer for device \""
