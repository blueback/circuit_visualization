...
private:
  ...
  std::vector<VkCommandBuffer> unpresentableRenderCommandBuffers;
  std::vector<VkCommandBuffer> unpresentableCopyCommandBuffers;
...
private:
  ...
  */
  ...
  void initVulkan() {
    ...
    unpresentableRenderCommandBuffers.resize(
        unpresentablePhysicalDevices.size());
    unpresentableCopyCommandBuffers.resize(unpresentablePhysicalDevices.size());
    ...
    for (uint32_t i = 0; i < unpresentablePhysicalDevices.size(); i++) {
      ...
      createCommandBuffer(
          unpresentablePhysicalDevices[i], unpresentableLogicalDevices[i],
          unpresentableCommandPools[i], unpresentableRenderCommandBuffers[i]);

      createCommandBuffer(
          unpresentablePhysicalDevices[i], unpresentableLogicalDevices[i],
          unpresentableCommandPools[i], unpresentableCopyCommandBuffers[i]);
  ...
  static void recordCommandBufferForCopyingToBuffer(
      VkPhysicalDevice physicalDevice, VkDevice logicalDevice,
  ...
  void drawFrame2(
      ...
      VkFramebuffer frameBuffer_unp, VkCommandBuffer renderCommandBuffer_unp,
      VkCommandBuffer copyCommandBuffer_unp,
      VkSemaphore renderingFinishedSemaphore_unp, VkFence interDeviceFence_unp,
      VkBuffer stagingBuffer_unp, void *stagingBufferData_unp,
    ...
    vkResetCommandBuffer(renderCommandBuffer_unp, 0);

    recordCommandBufferForUnpresentableDevice(
        physicalDevice_unp, renderCommandBuffer_unp, renderPass_unp,
        frameBuffer_unp, extent, graphicsPipeline_unp, false);
    ...
    renderSubmitInfo_unp.pCommandBuffers = &renderCommandBuffer_unp;
    VkSemaphore renderSignalSemaphores_unp[] = {renderingFinishedSemaphore_unp};
    renderSubmitInfo_unp.signalSemaphoreCount = 1;
    renderSubmitInfo_unp.pSignalSemaphores = renderSignalSemaphores_unp;

    if (vkQueueSubmit(graphicsQueue_unp, 1, &renderSubmitInfo_unp, nullptr) !=
        VK_SUCCESS) {
      throw std::runtime_error("failed to submit draw command buffer!");
    } else {
      std::cout << "Submitted draw command buffer for device \""
    ...
    //////////////////////////////////////////////////////////////////////
    ///                   START copy to host visible buffer
    //////////////////////////////////////////////////////////////////////

    vkResetCommandBuffer(copyCommandBuffer_unp, 0);

    recordCommandBufferForCopyingToBuffer(physicalDevice_unp, logicalDevice_unp,
                                          copyCommandBuffer_unp, image_unp,
                                          extent, stagingBuffer_unp);

    VkSubmitInfo copySubmitInfo_unp{};
    copySubmitInfo_unp.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
    VkSemaphore copyWaitSemaphores_unp[] = {renderingFinishedSemaphore_unp};
    VkPipelineStageFlags copyWaitStages_unp[] = {
        VK_PIPELINE_STAGE_TRANSFER_BIT};
    copySubmitInfo_unp.waitSemaphoreCount = 1;
    copySubmitInfo_unp.pWaitSemaphores = copyWaitSemaphores_unp;
    copySubmitInfo_unp.pWaitDstStageMask = copyWaitStages_unp;
    copySubmitInfo_unp.commandBufferCount = 1;
    copySubmitInfo_unp.pCommandBuffers = &copyCommandBuffer_unp;
    copySubmitInfo_unp.signalSemaphoreCount = 0;
    copySubmitInfo_unp.pSignalSemaphores = nullptr; // Optional

    if (vkQueueSubmit(graphicsQueue_unp, 1, &copySubmitInfo_unp,
  ...
  void mainLoop() {
    while (!glfwWindowShouldClose(window)) {
      ...
      drawFrame2(
          ...
          unpresentableRenderCommandBuffers[0],
          unpresentableCopyCommandBuffers[0],
